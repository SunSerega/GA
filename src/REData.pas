...
uses GData, glObjectData, CFData, TCData, OpenGL;
...
type
  ...
  CameraT = record
    ...
    const Speed = 0.4 * 10;//ToDo 0.7*PlayerH/Camera tps       =0.4
  ...
  FloorData = record
    ...
    public rot: real;
    public slope: SLine;
    public Friction: real;
    public SurfMtx: mtx3x3f;
    
    public class Empty: FloorData;
    
    public constructor create(rot: real; Friction: real; slope: SLine);
    begin
      self.rot := rot;
  ...
  ///Интерфейс для общий для комнат и отгруженых пустышек
  ISegment = interface
    ...
    function GetFloor(nCamera: CameraT): FloorData;
    
    function GetAllHB: List<HitBoxT>;
    
    {$endregion}
    
    {$region Draw}
    
    procedure Draw(dx, dy: Single);
    
    function DrawForMap(dx, dy, dz: Single): boolean;
    
    {$endregion}
    
    {$region Tick}
    
    procedure PlayerTick;
    
    procedure DangeonTick;
  
    {$endregion}
  
  end;
  ///Тип-заглушка для комнат, выгруженых из оперативки
  UnLoadedSegment = sealed class(ISegment)
    
    {$region реализация ISegment}
    
    {$region Room Data}
    
    public function ClassName := 'UnLoadedSegment';
    
    public function GetFloor(nCamera: CameraT) := FloorData.Empty;
    
    public function GetAllHB := new List<HitBoxT>;
    
    {$endregion}
    
    {$region Draw}
    
    public procedure Draw(dx, dy: Single) := raise new System.InvalidOperationException('Попытка нарисовать отгруженную комнату');
    
    public function DrawForMap(dx, dy, dz: Single) := false;
    
    {$endregion}
    
    {$region Tick}
    
    public procedure PlayerTick := raise new System.InvalidOperationException('Попытка нарисовать отгруженную комнату');
    
    public procedure DangeonTick := raise new System.InvalidOperationException('Попытка нарисовать отгруженную комнату');
  ...
  public 
    ...
    function Senter: vec2f;
    ///Добавляет этот проход к списку ожидающих генерации
    public procedure AddToWait;
    
    public constructor(Z: smallint; p1, p2: vec2f; Whose, ConTo: Segment);
  ...
  public 
    ...
    ///Возвращает название энтити. Обязано быть переопределено в каждом классе - энтити.
  ...
  ///Базовый тип комнат
  Segment = abstract class(ISegment)
  ...
  public 
    ...
    MapHitBox: List<HitBoxT>;//       -XY -Z -rot
    ...
    protected function RoomWait: List<ConnectionT>;
    ...
    public procedure AddToDangeon(D: Dangeon; Regs: List<vec3i>);
  ...
  public 
    ...
    MaxR: real;
    VRmlt: real;
    Regs: Dictionary<vec3i, List<Segment>>;
    ...
    public function SegmentsAt(p: vec3i): List<Segment>;
...
///[DEBUG] Float To Integer thru pointer
...
begin
  for i: integer := 0 to Self.Count - 1 do
    Self[i].HB := Self[i].HB.Rotate(rot);
  Result := Self;
end;
///Сдвигает хитбоксы соединений на вектор d
function AddToHB(Self: List<ConnectionT>; d: vec2f): List<ConnectionT>; extensionmethod;
begin
  for i: integer := 0 to Self.Count - 1 do
...
constructor ConnectionT.create(Z: smallint; p1, p2: vec2f; Whose, ConTo: Segment);
begin
  //TC += 1;
  //Log('TC:', TC);
...
function Segment.GetFloor(nCamera: CameraT) := new FloorData;

function Segment.GetAllHB := WallHitBox + Connections.Where(C -> C.Empty).ToList.ConvertAll(C -> C.HB - new vec2f(self.X, self.Y));
...
begin
  ...
  //Log3(RTD);
  with DangeonIn do
    foreach p: vec3i in Regs.Keys.ToArray do
    begin
...
begin
  ...
  foreach var E in Entities.ToArray do
    E.Draw;
  ...
  if LHShow and false then
  begin
    foreach var HB in GetAllHB do
      gr.Polygon(true, 1, 0, 0, 0.2, new vec3d[4](
            new vec3d(HB.p1.X, HB.p1.Y, +RW),
            new vec3d(HB.p2.X, HB.p2.Y, +RW),
            new vec3d(HB.p2.X, HB.p2.Y, -RW),
            new vec3d(HB.p1.X, HB.p1.Y, -RW)));
    foreach var C in Connections do
      with C do
        gr.Polygon(true, 0, 1, 0, 0.2, new vec3d[4](
            new vec3d(HB.p1.X - X, HB.p1.Y - Y, +RW),
            new vec3d(HB.p2.X - X, HB.p2.Y - Y, +RW),
            new vec3d(HB.p2.X - X, HB.p2.Y - Y, -RW),
            new vec3d(HB.p1.X - X, HB.p1.Y - Y, -RW)));
  end;
  
  if false then DrawDangeonRegs;
...
procedure Segment.AddToDangeon(D: Dangeon; Regs: List<vec3i>);
begin
  id := D.idused;
  D.idused += 1;
  DangeonIn := D;
        ...
        begin
          ...
          foreach var nC in S.Connections do
            if nC.ConTo = nil then
              if Z = nC.Z + S.Z then
                if Round(nC.HB.p1 + new vec2f(S.X, s.Y)) = p2 then
                  if Round(nC.HB.p2 + new vec2f(S.X, s.Y)) = p1 then
...
begin
  ...
  Pre.Connections.AddToHB(new vec2f(Pre.X, Pre.Y));
  foreach var C in Pre.Connections do
    C.Z += Pre.Z;
  
  Pre.MapHitBox.Rotate(Pre.rot);
  Pre.MapHitBox.AddToHB(new vec2f(Pre.X, Pre.Y));
  ...
  begin
    ...
    Pre.Regs := new List<vec3i>(l.Count * (Pre.ZMax - Pre.ZMin + 1));
    foreach var p in l do
      for var Z := Pre.ZMin to Pre.ZMax do
        Pre.Regs.Add(new vec3i(p.X, p.Y, Z));
...
function Dangeon.SegmentsAt(p: vec3i): List<Segment>;
begin
...
function Dangeon.SegmentsAt(X, Y: integer; Z: smallint) := SegmentsAt(new vec3i(X, Y, Z));
...
begin
  ...
  self.MaxR := MaxR;
  self.VRmlt := VRmlt;
  Regs := new Dictionary<vec3i, List<Segment>>;
