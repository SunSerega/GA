...
begin
  try
    Result := new Texture(w, h, Texture.GetPerlinNoisePaternBytes2(w, h, 0.5, 0.5, 0.5, 0.005 * CM2, 0.020 * CM2, 0.040 * CM2, 0.1, 3), false, true);
  except
...
begin
  ...
  var pc := Round(w * h / 40);
  pc := 1;
  var pts := new List<CPoint>(pc);
  loop pc do
    pts.Add(new CPoint(Random(w), Random(h), 0, 0.7 + Rand(0.0), 0.7 + Rand(0.0), 0.7 + Rand(0.0), 0));
...
begin
  ...
  var ba := new byte[w * h * 3];
  var pts := new List<PPoint>;
  loop Round(w * h / (300 + Random(21))) do
    //loop 5 do
    pts.Add(new PPoint(Random * (w + 60) - 30, Random * (h + 60) - 30, SrMRad + Rand(MRad)));
  while pts.Count <> 0 do
  begin
    var nP := new PPoint(0, 0, 0);
    foreach var P in pts do
      if P.Z > nP.Z then
        nP := P;
    pts.Remove(nP);
